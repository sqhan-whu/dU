	...
	with open(file_name) as f1:
		for line in f1:
			if not line.strip().startswith("#"):
				...
				if line[3] == "T" and line[4] == "C" and not line[9].startswith(".") and not line[10].startswith(".") or line[3] == "A" and line[4] == "G" and not line[9].startswith(".") and not line[10].startswith("."):
					inp = line[9].split(':')
					oup = line[10].split(':')
					inp_c = float(inp[6])
					oup_c = float(oup[6])
					...
					inp_t = inp_ref_cov/inp_cov
					oup_t = oup_ref_cov/oup_cov
					#print(inp_pval,inp_t,oup_c,inp_cov,oup_cov,inp_t/oup_t, oup_c/(inp_c+0.00001))
					if r == False and line1.split('\t')[9].startswith("0/0"):
						if inp_pval < pval and inp_t > min_T_ratio and oup_c < 50 and inp_cov >=20 and oup_cov >=20 and oup_t/inp_t < T_dev and oup_c/(inp_c+0.00001) > min_C_ratio:

							n +=1
							out_list.append([line[0],line[1],line[3],line[4],inp_cov,inp_ref_cov,inp_alt_cov,inp_t,oup_cov,oup_ref_cov,oup_alt_cov,oup_t,oup_t/inp_t,oup_c/(inp_c+0.00001)])
							pos.append(str(line[0])+"\t"+str(line[1]))

					elif r == True and line1.split('\t')[10].startswith("0/0"):
						if oup_pval < pval and oup_t > min_T_ratio and inp_c < 50 and inp_cov >=20 and oup_cov >=20 and inp_t/oup_t < T_dev and inp_c/(oup_c+0.00001) > min_C_ratio:
							n +=1
							out_list.append([line[0],line[1],line[3],line[4],inp_cov,inp_ref_cov,inp_alt_cov,inp_t,oup_cov,oup_ref_cov,oup_alt_cov,oup_t,inp_t/(oup_t+0.0001),inp_c/(oup_c+0.0001)])
							pos.append(str(line[0])+"\t"+str(line[1]))
	f1.close()			
