...
python script/summary_table.py -s 1=x -s 2=y --fasta test.fa
...
def summary_table(sample_names, bedgraph_filenames, fasta_filename, revcomp, verbose):
    ...
    headers = ['#chrom','start','end','strand','seq','have.signal','count.sum']
    headers.extend(sample_names)
    print ('\t'.join(headers))

    for els in zip(intersection_bedtool, fasta, *maps):
        ...
        if len(seq) > 1:
            ...
            for pos, nuc in zip(range(region.start, region.end), seq):

                fields = [region.chrom, pos, pos + 1,
                          strand, nuc, have_signal, count_sum]
                fields.extend(signals)
                print ('\t'.join(map(str, fields)))

        else:
            fields = [region.chrom, region.start, region.end,
                      strand, seq, have_signal, count_sum]
            fields.extend(signals)
